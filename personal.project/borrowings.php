...
if ($_SESSION['is_admin'] == 'true') {
    $sql = "SELECT books.book_title, users.email, books.id, borrowings.borrow_date, borrowings.return_date
            FROM books 
            INNER JOIN borrowings ON books.id = borrowings.book_id 
            INNER JOIN users ON users.id = borrowings.user_id";
    $selectBorrowings = $conn->prepare($sql);
    $selectBorrowings->execute();
} else {
    $sql = "SELECT books.book_title, users.email, books.id, borrowings.borrow_date, borrowings.return_date
            FROM books 
            INNER JOIN borrowings ON books.id = borrowings.book_id 
    ...
    $selectBorrowings = $conn->prepare($sql);
    $selectBorrowings->bindParam(':user_id', $user_id);
    $selectBorrowings->execute();
}

$borrowings_data = $selectBorrowings->fetchAll();
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      <div class="table-responsive">
        <table class="table table-striped table-sm">
          ...
          <tbody>
          <?php if ($_SESSION['is_admin'] == 'true') { ?>
            <?php foreach ($borrowings_data as $borrowing_data) { ?>

               <tr>
                <td><?php echo $borrowing_data['book_title']; ?></td>
                <td><?php echo $borrowing_data['email']; ?></td>
                <td><?php echo $borrowing_data['borrow_date']; ?></td>
                <td><?php echo $borrowing_data['return_date']; ?></td>

                <td><a href="approve.php?id=<?= $borrowing_data['id'];?>">Approve</a></td>
                <td><a href="decline.php?id=<?= $borrowing_data['id'];?>">Decline</a></td>
              </tr>
